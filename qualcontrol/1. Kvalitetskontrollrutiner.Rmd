...
```{r}
update_qcyear(2024) # Endre år ved behov
```
...
# 4. Sjekk aggregering mellom geonivåer og ukjent bydel
...
plot_boxplot(onlynew = TRUE, # Skal bare nye uteliggere markeres?
             change = FALSE, # Vanlige boksplott, ikke år-til-år
             save = TRUE)
```
...
```{r}
plot_boxplot(onlynew = TRUE, # Skal bare nye uteliggere markeres?
             change = TRUE, # Bestemmer at det er år-til-år
             save = TRUE)
```
...
plot_timeseries(onlynew = TRUE, # Skal bare tidsserier med nye uteliggere plottes?
                change = FALSE, # Vanlige tidsserier
                save = TRUE)
```
...
```{r}
plot_timeseries(onlynew = TRUE, # Skal bare tidsserier med nye uteliggere plottes?
                change = TRUE, # Tidsserier for år-til-år endringer
                save = TRUE)
```

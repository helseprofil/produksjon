...
```{r}
update_qcyear(2026) # Endre år ved behov
```
...
```{r}
readfiles(cube.new = "BEFVEKST_2026-06-05-13-17", # KUBENAVN_DATOTAG. Funksjonen leser fra QC-mappen om filen finnes der, ellers DATERT. 
          recode.new = FALSE, # Rekode til gjeldende GEO?
          cube.old = "BEFVEKST_2025-10-20-14-06", # Kan også være NULL
          ...
          dumps = NULL) # default er comparecube og newcube_flag. Kan også være NULL for ingen fildumper.
```
...
- OBS 2026/2027, med innføring av levekårssoner oversvømmes kolonnen med nye verdier for GEO. Disse kan derfor med fordel filtreres ut.

```{r}
compare_dimensions(cube.new = newcube[GEOniv != "V"], 
                   cube.old = oldcube[GEOniv != "V"])
```
...
## Sjekke hvilket nivå av en dimensjon som har differ i en verdikolonne
- Om du velger kolonner som ikke finnes i filen får du advarsel
- Kan være nyttig for å se om det bare er GEO-koder med grenseendringer som har nevnerdiffer f.eks.
```{r}
# Eksempel: hvilke geokoder har differ for sumteller(_uprikk)
diffvals_which_levels(dt = comparecube, dim = "GEO", val = "sumTELLER_uprikk") 
# Eksempel: hvilke utdanningsnivå har differ for MEIS
diffvals_which_levels(dt = comparecube, dim = "KJONN", val = "RATE") 
...
## Er det tall under prikkegrensen?
 ...
 - Viser også prikkegrensene som er satt i ACCESS *akkurat nå, ikke nødvendigvis når kuben ble kjørt!!*
...
## Sjekk rader som er ulikt prikket
- Bruker de nye prikkekolonnene til å vise hvorfor en rad er prikket
- Er en ny eller utgått prikk forklart av ulik nabo- eller serieprikking?
- Bruk View() for å kunne sortere i tabellen
  - Output kan filtreres, bruk de utkommenterte radene som utgangspunkt.
  - `SPVFLAGG_DIFF` er SPV_ny - SPV_gammel, < 0 = utgått, > 0 = ny prikk. 

```{r}
View(explore_different_censoring())

# View(explore_different_censoring()[SPVFLAGG_DIFF > 0]) # Bare nye prikker
# View(explore_different_censoring()[SPVFLAGG_DIFF < 0 & KJONN == 0]) # Bare utgåtte prikker
# View(explore_different_censoring()[SPVFLAGG_DIFF < 0 & ALDER == ... & KJONN == ... & AAR == ...]) # Filtrere ut ønskede strata
...
- Om undernivået > overnivået er allerede sjekket i kubekjøringen, men de eksakte tallene vises her.
...
# ---
# EKSTRA: spesialsjekker og diverse ekstra

## BEFVEKST
```{r}
check_befvekst(statusfolder = "KUBESTATUS_2027")
```
...
- Ukjent bydel > grensene som er satt i khfunctions er allerede skjult med SPVFLAGG = 1 og dekningprikket = 1
...
### Viser beregning på alle strata med komplette ikkeprikkede tall
...
### Sjekker bare strata som er uprikket

```{r}
unknown_bydel(type = "censored", # "missing" eller "censored"
              crop = TRUE, # Skal bare de strata med størst andel ukjent bydel vises? (tabellen kan bli veldig stor hvis ikke)
              maxrows = 4000, # Maksimalt antall rader i tabellen (fordeles jevnt på KOMMUNE og TARGET)
              save = F) # Hele tabellen lagres som csv
```

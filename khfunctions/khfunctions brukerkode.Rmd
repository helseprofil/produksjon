...
```{r}
LagKUBE("ALKOHOL_UNGDATA", write = TRUE, alarm = FALSE, dumps = list()) 
```

## Manuell gjennomgang av KUBE-output

- RESULTAT$QC = kvalkontrollkube
- RESULTAT$ALLVIS = filen som publiseres
- Disse kan filtreres med vanlig data.table-kode
- Bruk View() for å få sorterbar tabell i eget vindu

- Prikkekolonner = orgprikket, pvern, serieprikket, naboprikket, dekningprikket
- Filtrere på GEOniv i QC-fil: 
    - GEO > 999999 = levekårssone
    - GEO > 9999 & GEO < 999999 = bydel
    - grepl("^4601", GEO) = alle geokoder som starter med 4601, om man vil se alle LKS i Bergen. 

Kodeeksempler:    
```{r}
# RESULTAT$QC[dekningprikket == 1, unique(GEO)] # hvilke geokoder har blitt dekningprikket
# View(RESULTAT$QC[dekningprikket == 1]) # Vis tabell over alle rader som har blitt dekningprikket, 
# View(RESULTAT$QC[dekningprikket == 1 & KJONN == ..., & AAR == ....]) # Legge til flere filtre
# RESULTAT$QC[dekningprikket == 1, .N, by = KJONN] # Kan se antall rader som er dekningprikket per kategoro (f.eks. kjønn, utdanning..)
# RESULTAT$QC[grepl("^4601", GEO)] # Finn alle rader med GEO som starter på 4601
```
...
- Fra khfunctions 1.1.5 kjører denne automatisk friskviksjekk fra qualcontrol, slik at alle filene blir sjekket for feil
- For å endre årgang kan man bruke argumentet year = ønsket år. Default er produksjonsåret som ligger i options.
```{r}
make_godkjent_folder(profil = "FHP", # ELLER "OVP"
                     geoniv = "K") # "K", "B" eller "F"
```
